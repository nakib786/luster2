...
export const TestimonialStack = ({ testimonials, visibleBehind = 2 }: TestimonialStackProps) => {
  ...
  const handleDragStart = (e: React.MouseEvent | React.TouchEvent, index: number) => {
    ...
    setIsDragging(true);
    pauseAutoSwipe(); // Pause auto-swipe when user starts dragging
  ...
  }, [isDragging, dragOffset, activeIndex, navigate, resumeAutoSwipe]);
        ...
        {testimonials.map((_, index) => (
          <button 
            ...
            onClick={() => {
              navigate(index);
              pauseAutoSwipe();
              setTimeout(() => resumeAutoSwipe(), 2000);
            }} 
            onMouseEnter={pauseAutoSwipe}
            onMouseLeave={resumeAutoSwipe}
